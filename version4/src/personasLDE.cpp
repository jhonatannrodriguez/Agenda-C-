...
void insertarInicioDeTPersonasLDE(TPersonasLDE &personas, TPersona persona){
       Nodoper person = new nuevo;  
       person->persona = copiarTPersona(persona);
       liberarTPersona(persona);
         if(personas->prim == NULL){
		   person->sig = person->ant =NULL;
		   personas->prim = personas->ult = person;
          }
          else{
       person-> sig = personas->prim;
       person->ant = NULL;
...
void eliminarPersonaConNombreTPersonasLDE(TPersonasLDE &personas, const char nombre[100]){
    Nodoper person = personas->prim;
    while(person != NULL && strcmp(nombre, nombreTPersona(person->persona)) != 0)
        person = person->sig;
     if (person == personas->prim && person == personas->ult) {
            personas->prim = personas->ult = NULL;
        } 
        else if(person == personas->prim){
            personas->prim = person->sig;
            person->sig->ant = NULL;
        }

        else if (person == personas->ult) {
            person->ant->sig = NULL;
            personas->ult = person->ant;
        } 
        else {
            person->sig->ant = person->ant;
            person->ant->sig = person->sig;
        }
        personas->pos--;
        person->sig = person->ant = NULL;
        liberarTPersona(person->persona);
           delete person;
}
